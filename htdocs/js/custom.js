...
jQuery(document).ready(function(){	
	...
	jQuery('#showpost-'+invariable[0]+' .showpostpostcontent').html('<div class="loading"></div>');
	if(jQuery('#showpost-'+invariable[0]+' .main').is(":visible")){
		...
		jQuery('#showpost-'+invariable[0]).delay(300).animate({"height": "695px"}, 500);
			jQuery('#showpost-'+invariable[0]+' .showpostpostcontent').load(url+'/single_home'+type+'.php',{ 'id': invariable[1], 'type': invariable[0] } ,function ( response, status, xhr ) {
	...
	jQuery('.closehomeshow-'+invariable[0]).click(function() {
		...
		jQuery('#showpost-'+invariable[0]).animate({"height": "-="+height+"px"}, 200);
...
jQuery(document).ready(function(){
	...
	if (jQuery( ".pagenav.home" ).length) {
		jQuery( ".pagenav.home .menu a" ).each(function() {
				...
				jQuery( window ).scroll(function() {
					...
					if(jQuery(id).isOnScreen()){
						...
						if(document.getElementById(newid[1]).getBoundingClientRect().top < 250){
							...
							jQuery( ".pagenav.home > li a[href='"+id+"']" ).removeClass('important_color');				
						}
...
jQuery('html, body').animate({scrollTop:jQuery('#remove').offset().top-120}, 400);
...
jQuery("#headerwrap").addClass('smallHeader');
}
else{
jQuery("#header").removeClass('smallHeader');
jQuery("#headerwrap").removeClass('smallHeader');
}
});
}
