 ...
 */
function get_the_term_list( $id, $taxonomy, $before = '', $sep = '', $after = '', $deleteCat = false ) {
	...
	foreach ( $terms as $term ) {
		...
		$validCat = true;
		$termName = $term->name;
		if($taxonomy == 'portfoliocategory' && $term->term_id == 54 && $deleteCat){ $validCat = false;}
		elseif($taxonomy == 'portfoliocategory' && $term->term_id == 57 && $deleteCat){ $validCat = false;}
		elseif($taxonomy == 'portfoliocategory' && $deleteCat){ $termName = str_replace('hasta','stand',$term->name);}
		if($validCat) $term_links[] = '<a href="' . esc_url( $link ) . '" rel="tag">' . $termName . '</a>';
	}
