...
function pmc_portfolio($portSize, $item, $post = 'port' ,$number = 0,$cat = '',$port_ajax){
	...
	if($cat != ''){	
			$args = array(
			'tax_query' => array(array('taxonomy' => $postC,'field' => 'id','terms' => $cat,'order'=>'date DESC')),
			'showposts'     => $showposts,
			'post_type' => $postT,
			'paged'    => $paged
			);
	...
	<div class="portfolio">	
		<div id="portitems4">
			...
			while ( have_posts() ) : the_post();
				...
				//data-order="'.str_replace(' ','',get_the_date(get_the_id())).get_the_id().'"	

				//echo get_the_status(get_the_id());
				...
				?>
						...
						<?php } ?>
							...
							<div class="recentdescription">
								...
								<div class="home-portfolio-categories"><?php echo get_the_term_list( get_the_ID(), 'portfoliocategory', '', ', ', '', true ); ?></div>
							</div>
						<?php if ($port_ajax == 'true'){ ?>	
						</div>
						<?php } ?>
